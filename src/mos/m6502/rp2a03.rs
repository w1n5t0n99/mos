...
impl Rp2a03 {
    ...
    pub fn tick(&mut self, bus: &mut dyn Bus, mut pinout: Pinout) -> Pinout {
        match u16::from(self.cpu.ir) {
            ...
            0x2502 =>  pinout = zeropage_read_c2::<And>(&mut self.cpu, bus, pinout),
            ...
            0x3503 =>  pinout = zeropage_x_read_c3::<And>(&mut self.cpu, bus, pinout),
            // And absolute read
            ...
            0x2D03 =>  pinout = absolute_read_c3::<And>(&mut self.cpu, bus, pinout),
            // And absolute x read
            ...
            0x3D04 =>  pinout = absolute_x_read_c4::<And>(&mut self.cpu, bus, pinout),
            // And absolute y read
            ...
            0x3904 =>  pinout = absolute_y_read_c4::<And>(&mut self.cpu, bus, pinout),
            // And indirect x read
            ...
            0x2105 =>  pinout = indirect_x_read_c5::<And>(&mut self.cpu, bus, pinout),
            // And indirect y read
            ...
            0x3105 =>  pinout = indirect_y_read_c5::<And>(&mut self.cpu, bus, pinout),
            // AslAccum single byte
            0x0A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x0A01 =>  pinout = single_byte_c1::<AslAccum>(&mut self.cpu, bus, pinout),
            ...
            0x0602 => pinout = zeropage_modify_c2::<Asl>(&mut self.cpu, bus, pinout),
            ...
            0x1603 => pinout = zeropage_x_modify_c3::<Asl>(&mut self.cpu, bus, pinout),
            ...
            0x0E03 => pinout = absolute_modify_c3::<Asl>(&mut self.cpu, bus, pinout),
            ...
            0x1E04 => pinout = absolute_x_modify_c4::<Asl>(&mut self.cpu, bus, pinout),
            ...
            0x9001 =>  pinout = branch_c1::<Bcc>(&mut self.cpu, bus, pinout),
            ...
            0xB001 =>  pinout = branch_c1::<Bcs>(&mut self.cpu, bus, pinout),
            ...
            0xF001 =>  pinout = branch_c1::<Beq>(&mut self.cpu, bus, pinout),
            ...
            0x3001 =>  pinout = branch_c1::<Bmi>(&mut self.cpu, bus, pinout),
            ...
            0xD001 =>  pinout = branch_c1::<Bne>(&mut self.cpu, bus, pinout),
            ...
            0x1001 =>  pinout = branch_c1::<Bpl>(&mut self.cpu, bus, pinout),
            ...
            0x5001 =>  pinout = branch_c1::<Bvc>(&mut self.cpu, bus, pinout),
            ...
            0x7001 =>  pinout = branch_c1::<Bvs>(&mut self.cpu, bus, pinout),
            ...
            0x2402 =>  pinout = zeropage_read_c2::<Bit>(&mut self.cpu, bus, pinout),
            // Bit absolute read
            ...
            0x2C03 =>  pinout = absolute_read_c3::<Bit>(&mut self.cpu, bus, pinout),
            // Clc single byte
            0x1800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x1801 =>  pinout = single_byte_c1::<Clc>(&mut self.cpu, bus, pinout),
            // Cld single byte
            0xD800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xD801 =>  pinout = single_byte_c1::<Cld>(&mut self.cpu, bus, pinout),
            // Cli single byte
            0x5800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x5801 =>  pinout = single_byte_c1::<Cli>(&mut self.cpu, bus, pinout),
            // Clv single byte
            0xB800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xB801 =>  pinout = single_byte_c1::<Clv>(&mut self.cpu, bus, pinout),
            // Cmp immediate
            0xC900 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xC901 =>  pinout = immediate_read_c1::<Cmp>(&mut self.cpu, bus, pinout),
            // Cmp zero page read
            0xC500 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0xC501 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0xC502 =>  pinout = zeropage_read_c2::<Cmp>(&mut self.cpu, bus, pinout),
            ...
            0xD503 =>  pinout = zeropage_x_read_c3::<Cmp>(&mut self.cpu, bus, pinout),
            // Cmp absolute read
            ...
            0xCD03 =>  pinout = absolute_read_c3::<Cmp>(&mut self.cpu, bus, pinout),
            // Cmp absolute x read
            ...
            0xDD04 =>  pinout = absolute_x_read_c4::<Cmp>(&mut self.cpu, bus, pinout),
            // Cmp absolute y read
            ...
            0xD904 =>  pinout = absolute_y_read_c4::<Cmp>(&mut self.cpu, bus, pinout),
            // Cmp indirect x read
            ...
            0xC105 =>  pinout = indirect_x_read_c5::<Cmp>(&mut self.cpu, bus, pinout),
            // Cmp indirect y read
            ...
            0xD105 =>  pinout = indirect_y_read_c5::<Cmp>(&mut self.cpu, bus, pinout),
            // Ldx immediate
            0xA200 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xA201 =>  pinout = immediate_read_c1::<Ldx>(&mut self.cpu, bus, pinout),
            // Ldx zero page read
            0xA600 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0xA601 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0xA602 =>  pinout = zeropage_read_c2::<Ldx>(&mut self.cpu, bus, pinout),
            ...
            0xB603 =>  pinout = zeropage_y_read_c3::<Ldx>(&mut self.cpu, bus, pinout),
            // Ldx absolute read
            ...
            0xAE03 =>  pinout = absolute_read_c3::<Ldx>(&mut self.cpu, bus, pinout),
            // Ldx absolute y read
            ...
            0xBE04 =>  pinout = absolute_y_read_c4::<Ldx>(&mut self.cpu, bus, pinout),
            // Ldy immediate
            0xA000 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xA001 =>  pinout = immediate_read_c1::<Ldy>(&mut self.cpu, bus, pinout),
            // Ldy zero page read
            0xA400 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0xA401 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0xA402 =>  pinout = zeropage_read_c2::<Ldy>(&mut self.cpu, bus, pinout),
            ...
            0xB403 =>  pinout = zeropage_x_read_c3::<Ldy>(&mut self.cpu, bus, pinout),
            // Ldy absolute read
            ...
            0xAC03 =>  pinout = absolute_read_c3::<Ldy>(&mut self.cpu, bus, pinout),
            // Ldy absolute x read
            ...
            0xBC04 =>  pinout = absolute_x_read_c4::<Ldy>(&mut self.cpu, bus, pinout),
            // Cpx immediate
            0xE000 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xE001 =>  pinout = immediate_read_c1::<Cpx>(&mut self.cpu, bus, pinout),
            // Cpx zero page read
            0xE400 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0xE401 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0xE402 =>  pinout = zeropage_read_c2::<Cpx>(&mut self.cpu, bus, pinout),
            // Cpx absolute read
            ...
            0xEC03 =>  pinout = absolute_read_c3::<Cpx>(&mut self.cpu, bus, pinout),
            // Cpy immediate
            0xC000 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xC001 =>  pinout = immediate_read_c1::<Cpy>(&mut self.cpu, bus, pinout),
            // Cpy zero page read
            0xC400 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0xC401 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0xC402 =>  pinout = zeropage_read_c2::<Cpy>(&mut self.cpu, bus, pinout),
            // Cpy absolute read
            ...
            0xCC03 =>  pinout = absolute_read_c3::<Cpy>(&mut self.cpu, bus, pinout),
            ...
            0xC602 => pinout = zeropage_modify_c2::<Dec>(&mut self.cpu, bus, pinout),
            ...
            0xD603 => pinout = zeropage_x_modify_c3::<Dec>(&mut self.cpu, bus, pinout),
            ...
            0xCE03 => pinout = absolute_modify_c3::<Dec>(&mut self.cpu, bus, pinout),
            ...
            0xDE04 => pinout = absolute_x_modify_c4::<Dec>(&mut self.cpu, bus, pinout),
            ...
            0xCA01 =>  pinout = single_byte_c1::<Dex>(&mut self.cpu, bus, pinout),
            // Dey single byte
            0x8800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x8801 =>  pinout = single_byte_c1::<Dey>(&mut self.cpu, bus, pinout),
            // Eor immediate
            0x4900 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x4901 =>  pinout = immediate_read_c1::<Eor>(&mut self.cpu, bus, pinout),
            // Eor zero page read
            0x4500 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0x4501 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0x4502 =>  pinout = zeropage_read_c2::<Eor>(&mut self.cpu, bus, pinout),
            ...
            0x5503 =>  pinout = zeropage_x_read_c3::<Eor>(&mut self.cpu, bus, pinout),
            // Eor absolute read
            ...
            0x4D03 =>  pinout = absolute_read_c3::<Eor>(&mut self.cpu, bus, pinout),
            // Eor absolute x read
            ...
            0x5D04 =>  pinout = absolute_x_read_c4::<Eor>(&mut self.cpu, bus, pinout),
            // Eor absolute y read
            ...
            0x5904 =>  pinout = absolute_y_read_c4::<Eor>(&mut self.cpu, bus, pinout),
            // Eor indirect x read
            ...
            0x4105 =>  pinout = indirect_x_read_c5::<Eor>(&mut self.cpu, bus, pinout),
            // Eor indirect y read
            ...
            0x5105 =>  pinout = indirect_y_read_c5::<Eor>(&mut self.cpu, bus, pinout),
            ...
            0xE602 => pinout = zeropage_modify_c2::<Inc>(&mut self.cpu, bus, pinout),
            ...
            0xF603 => pinout = zeropage_x_modify_c3::<Inc>(&mut self.cpu, bus, pinout),
            ...
            0xEE03 => pinout = absolute_modify_c3::<Inc>(&mut self.cpu, bus, pinout),
            ...
            0xFE04 => pinout = absolute_x_modify_c4::<Inc>(&mut self.cpu, bus, pinout),
            ...
            0xE801 =>  pinout = single_byte_c1::<Inx>(&mut self.cpu, bus, pinout),
            // Iny single byte
            0xC800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xC801 =>  pinout = single_byte_c1::<Iny>(&mut self.cpu, bus, pinout),
            // Lda immediate
            0xA900 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xA901 =>  pinout = immediate_read_c1::<Lda>(&mut self.cpu, bus, pinout),
            // Lda zero page read
            0xA500 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0xA501 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0xA502 =>  pinout = zeropage_read_c2::<Lda>(&mut self.cpu, bus, pinout),
            ...
            0xB503 =>  pinout = zeropage_x_read_c3::<Lda>(&mut self.cpu, bus, pinout),
            // Lda absolute read
            ...
            0xAD03 =>  pinout = absolute_read_c3::<Lda>(&mut self.cpu, bus, pinout),
            // Lda absolute x read
            ...
            0xBD04 =>  pinout = absolute_x_read_c4::<Lda>(&mut self.cpu, bus, pinout),
            // Lda absolute y read
            ...
            0xB904 =>  pinout = absolute_y_read_c4::<Lda>(&mut self.cpu, bus, pinout),
            // Lda indirect x read
            ...
            0xA105 =>  pinout = indirect_x_read_c5::<Lda>(&mut self.cpu, bus, pinout),
            // Lda indirect y read
            ...
            0xB105 =>  pinout = indirect_y_read_c5::<Lda>(&mut self.cpu, bus, pinout),
            // LsrAccum single byte
            0x4A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x4A01 =>  pinout = single_byte_c1::<LsrAccum>(&mut self.cpu, bus, pinout),
            ...
            0x4602 => pinout = zeropage_modify_c2::<Lsr>(&mut self.cpu, bus, pinout),
            ...
            0x5603 => pinout = zeropage_x_modify_c3::<Lsr>(&mut self.cpu, bus, pinout),
            ...
            0x4E03 => pinout = absolute_modify_c3::<Lsr>(&mut self.cpu, bus, pinout),
            ...
            0x5E04 => pinout = absolute_x_modify_c4::<Lsr>(&mut self.cpu, bus, pinout),
            ...
            0xEA01 =>  pinout = single_byte_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Ora immediate
            0x0900 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x0901 =>  pinout = immediate_read_c1::<Ora>(&mut self.cpu, bus, pinout),
            // Ora zero page read
            0x0500 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0x0501 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0x0502 =>  pinout = zeropage_read_c2::<Ora>(&mut self.cpu, bus, pinout),
            ...
            0x1503 =>  pinout = zeropage_x_read_c3::<Ora>(&mut self.cpu, bus, pinout),
            // Ora absolute read
            ...
            0x0D03 =>  pinout = absolute_read_c3::<Ora>(&mut self.cpu, bus, pinout),
            // Ora absolute x read
            ...
            0x1D04 =>  pinout = absolute_x_read_c4::<Ora>(&mut self.cpu, bus, pinout),
            // Ora absolute y read
            ...
            0x1904 =>  pinout = absolute_y_read_c4::<Ora>(&mut self.cpu, bus, pinout),
            // Ora indirect x read
            ...
            0x0105 =>  pinout = indirect_x_read_c5::<Ora>(&mut self.cpu, bus, pinout),
            // Ora indirect y read
            ...
            0x1105 =>  pinout = indirect_y_read_c5::<Ora>(&mut self.cpu, bus, pinout),
            // RolAccum single byte
            0x2A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x2A01 =>  pinout = single_byte_c1::<RolAccum>(&mut self.cpu, bus, pinout),
            ...
            0x2602 => pinout = zeropage_modify_c2::<Rol>(&mut self.cpu, bus, pinout),
            ...
            0x3603 => pinout = zeropage_x_modify_c3::<Rol>(&mut self.cpu, bus, pinout),
            ...
            0x2E03 => pinout = absolute_modify_c3::<Rol>(&mut self.cpu, bus, pinout),
            ...
            0x3E04 => pinout = absolute_x_modify_c4::<Rol>(&mut self.cpu, bus, pinout),
            ...
            0x6A01 =>  pinout = single_byte_c1::<RorAccum>(&mut self.cpu, bus, pinout),
            ...
            0x6602 => pinout = zeropage_modify_c2::<Ror>(&mut self.cpu, bus, pinout),
            ...
            0x7603 => pinout = zeropage_x_modify_c3::<Ror>(&mut self.cpu, bus, pinout),
            ...
            0x6E03 => pinout = absolute_modify_c3::<Ror>(&mut self.cpu, bus, pinout),
            ...
            0xF801 =>  pinout = single_byte_c1::<Sed>(&mut self.cpu, bus, pinout),
            // Sei single byte
            0x7800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x7801 =>  pinout = single_byte_c1::<Sei>(&mut self.cpu, bus, pinout),
            // Sta zero page store
            0x8500 =>  pinout = zeropage_store_c0(&mut self.cpu, bus, pinout),
            0x8501 => pinout = zeropage_store_c1::<Sta>(&mut self.cpu, bus, pinout),
            ...
            0x9502 => pinout = zeropage_x_store_c2::<Sta>(&mut self.cpu, bus, pinout),
            ...
            0x8D02 => pinout = absolute_store_c2::<Sta>(&mut self.cpu, bus, pinout),
            ...
            0x9D03 => pinout = absolute_x_store_c3::<Sta>(&mut self.cpu, bus, pinout),
            ...
            0x9903 => pinout = absolute_y_store_c3::<Sta>(&mut self.cpu, bus, pinout),
            ...
            0x8104 => pinout = indirect_x_store_c4::<Sta>(&mut self.cpu, bus, pinout),
            ...
            0x9104 => pinout = indirect_y_store_c4::<Sta>(&mut self.cpu, bus, pinout),
            0x9105 =>  pinout = indirect_y_store_c5(&mut self.cpu, bus, pinout),
            // Stx zero page store
            0x8600 =>  pinout = zeropage_store_c0(&mut self.cpu, bus, pinout),
            0x8601 => pinout = zeropage_store_c1::<Stx>(&mut self.cpu, bus, pinout),
            ...
            0x9602 => pinout = zeropage_y_store_c2::<Stx>(&mut self.cpu, bus, pinout),
            ...
            0x8E02 => pinout = absolute_store_c2::<Stx>(&mut self.cpu, bus, pinout),
            0x8E03 =>  pinout = absolute_store_c3(&mut self.cpu, bus, pinout),
            // Sty zero page store
            0x8400 =>  pinout = zeropage_store_c0(&mut self.cpu, bus, pinout),
            0x8401 => pinout = zeropage_store_c1::<Sty>(&mut self.cpu, bus, pinout),
            ...
            0x9402 => pinout = zeropage_x_store_c2::<Sty>(&mut self.cpu, bus, pinout),
            ...
            0x8C02 => pinout = absolute_store_c2::<Sty>(&mut self.cpu, bus, pinout),
            0x8C03 =>  pinout = absolute_store_c3(&mut self.cpu, bus, pinout),
            // Tax single byte
            0xAA00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xAA01 =>  pinout = single_byte_c1::<Tax>(&mut self.cpu, bus, pinout),
            // Tay single byte
            0xA800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xA801 =>  pinout = single_byte_c1::<Tay>(&mut self.cpu, bus, pinout),
            // Tsx single byte
            0xBA00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xBA01 =>  pinout = single_byte_c1::<Tsx>(&mut self.cpu, bus, pinout),
            // Txa single byte
            0x8A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x8A01 =>  pinout = single_byte_c1::<Txa>(&mut self.cpu, bus, pinout),
            // Txs single byte
            0x9A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x9A01 =>  pinout = single_byte_c1::<Txs>(&mut self.cpu, bus, pinout),
            // Tya single byte
            0x9800 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x9801 =>  pinout = single_byte_c1::<Tya>(&mut self.cpu, bus, pinout),
            // Aac immediate
            0x0B00 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x0B01 =>  pinout = immediate_read_c1::<Aac>(&mut self.cpu, bus, pinout),
            // Aac immediate
            0x2B00 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x2B01 =>  pinout = immediate_read_c1::<Aac>(&mut self.cpu, bus, pinout),
            // Arr immediate
            0x6B00 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x6B01 =>  pinout = immediate_read_c1::<Arr>(&mut self.cpu, bus, pinout),
            // Asr immediate
            0x4B00 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x4B01 =>  pinout = immediate_read_c1::<Asr>(&mut self.cpu, bus, pinout),
            // Atx immediate
            0xAB00 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xAB01 =>  pinout = immediate_read_c1::<Atx>(&mut self.cpu, bus, pinout),
            // Axs immediate
            0xCB00 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xCB01 =>  pinout = immediate_read_c1::<Axs>(&mut self.cpu, bus, pinout),
            // Aax zero page store
            0x8700 =>  pinout = zeropage_store_c0(&mut self.cpu, bus, pinout),
            0x8701 => pinout = zeropage_store_c1::<Aax>(&mut self.cpu, bus, pinout),
            ...
            0x9702 => pinout = zeropage_y_store_c2::<Aax>(&mut self.cpu, bus, pinout),
            ...
            0x8304 => pinout = indirect_x_store_c4::<Aax>(&mut self.cpu, bus, pinout),
            ...
            0x8F02 => pinout = absolute_store_c2::<Aax>(&mut self.cpu, bus, pinout),
            ...
            0x9F03 => pinout = absolute_y_store_c3::<Axa>(&mut self.cpu, bus, pinout),
            ...
            0x9304 => pinout = indirect_y_store_c4::<Axa>(&mut self.cpu, bus, pinout),
            ...
            0xC702 => pinout = zeropage_modify_c2::<Dcp>(&mut self.cpu, bus, pinout),
            ...
            0xD703 => pinout = zeropage_x_modify_c3::<Dcp>(&mut self.cpu, bus, pinout),
            ...
            0xCF03 => pinout = absolute_modify_c3::<Dcp>(&mut self.cpu, bus, pinout),
            ...
            0xDF04 => pinout = absolute_x_modify_c4::<Dcp>(&mut self.cpu, bus, pinout),
            ...
            0xDB04 => pinout = undoc_absolute_y_c4::<Dcp>( &mut self.cpu, bus, pinout),
            ...
            0xC305 => pinout = undoc_indirect_x_c5::<Dcp>( &mut self.cpu, bus, pinout),
            ...
            0xD305 => pinout = undoc_indirect_y_c5::<Dcp>( &mut self.cpu, bus, pinout),
            ...
            0x1403 =>  pinout = zeropage_x_read_c3::<Nop>(&mut self.cpu, bus, pinout),
            // Nop zero page x read
            ...
            0x3403 =>  pinout = zeropage_x_read_c3::<Nop>(&mut self.cpu, bus, pinout),
            // Nop zero page x read
            ...
            0x5403 =>  pinout = zeropage_x_read_c3::<Nop>(&mut self.cpu, bus, pinout),
            // Nop zero page x read
            ...
            0x7403 =>  pinout = zeropage_x_read_c3::<Nop>(&mut self.cpu, bus, pinout),
            // Nop zero page x read
            ...
            0xD403 =>  pinout = zeropage_x_read_c3::<Nop>(&mut self.cpu, bus, pinout),
            // Nop zero page x read
            ...
            0xF403 =>  pinout = zeropage_x_read_c3::<Nop>(&mut self.cpu, bus, pinout),
            // Nop immediate
            0x8000 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x8001 =>  pinout = immediate_read_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop immediate
            0x8200 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x8201 =>  pinout = immediate_read_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop immediate
            0x8900 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x8901 =>  pinout = immediate_read_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop immediate
            0xC200 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xC201 =>  pinout = immediate_read_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop immediate
            0xE200 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0xE201 =>  pinout = immediate_read_c1::<Nop>(&mut self.cpu, bus, pinout),
            ...
            0xE702 => pinout = zeropage_modify_c2::<Isc>(&mut self.cpu, bus, pinout),
            ...
            0xF703 => pinout = zeropage_x_modify_c3::<Isc>(&mut self.cpu, bus, pinout),
            ...
            0xEF03 => pinout = absolute_modify_c3::<Isc>(&mut self.cpu, bus, pinout),
            ...
            0xFF04 => pinout = absolute_x_modify_c4::<Isc>(&mut self.cpu, bus, pinout),
            ...
            0xFB04 => pinout = undoc_absolute_y_c4::<Isc>( &mut self.cpu, bus, pinout),
            ...
            0xE305 => pinout = undoc_indirect_x_c5::<Isc>( &mut self.cpu, bus, pinout),
            ...
            0xF305 => pinout = undoc_indirect_y_c5::<Isc>( &mut self.cpu, bus, pinout),
            ...
            0x0201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0x1200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x1201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0x2200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x2201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0x3200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x3201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0x4200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x4201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0x5200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x5201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0x6200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x6201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0x7200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x7201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0x9200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x9201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0xB200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xB201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0xD200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xD201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Kil single byte
            0xF200 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xF201 =>  pinout = single_byte_c1::<Kil>(&mut self.cpu, bus, pinout),
            // Lar absolute y read
            ...
            0xBB04 =>  pinout = absolute_y_read_c4::<Lar>(&mut self.cpu, bus, pinout),
            // Lax zero page read
            0xA700 =>  pinout = zeropage_read_c0(&mut self.cpu, bus, pinout),
            0xA701 =>  pinout = zeropage_read_c1(&mut self.cpu, bus, pinout),
            0xA702 =>  pinout = zeropage_read_c2::<Lax>(&mut self.cpu, bus, pinout),
            ...
            0xB703 =>  pinout = zeropage_y_read_c3::<Lax>(&mut self.cpu, bus, pinout),
            // Lax absolute read
            ...
            0xAF03 =>  pinout = absolute_read_c3::<Lax>(&mut self.cpu, bus, pinout),
            // Lax absolute y read
            ...
            0xBF04 =>  pinout = absolute_y_read_c4::<Lax>(&mut self.cpu, bus, pinout),
            // Lax indirect x read
            ...
            0xA305 =>  pinout = indirect_x_read_c5::<Lax>(&mut self.cpu, bus, pinout),
            // Lax indirect y read
            ...
            0xB305 =>  pinout = indirect_y_read_c5::<Lax>(&mut self.cpu, bus, pinout),
            // Nop single byte
            0x1A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x1A01 =>  pinout = single_byte_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop single byte
            0x3A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x3A01 =>  pinout = single_byte_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop single byte
            0x5A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x5A01 =>  pinout = single_byte_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop single byte
            0x7A00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0x7A01 =>  pinout = single_byte_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop single byte
            0xDA00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xDA01 =>  pinout = single_byte_c1::<Nop>(&mut self.cpu, bus, pinout),
            // Nop single byte
            0xFA00 =>  pinout = single_byte_c0(&mut self.cpu, bus, pinout),
            0xFA01 =>  pinout = single_byte_c1::<Nop>(&mut self.cpu, bus, pinout),
            ...
            0x2702 => pinout = zeropage_modify_c2::<Rla>(&mut self.cpu, bus, pinout),
            ...
            0x3703 => pinout = zeropage_x_modify_c3::<Rla>(&mut self.cpu, bus, pinout),
            ...
            0x2F03 => pinout = absolute_modify_c3::<Rla>(&mut self.cpu, bus, pinout),
            ...
            0x3F04 => pinout = absolute_x_modify_c4::<Rla>(&mut self.cpu, bus, pinout),
            ...
            0x3B04 => pinout = undoc_absolute_y_c4::<Rla>( &mut self.cpu, bus, pinout),
            ...
            0x2305 => pinout = undoc_indirect_x_c5::<Rla>( &mut self.cpu, bus, pinout),
            ...
            0x3305 => pinout = undoc_indirect_y_c5::<Rla>( &mut self.cpu, bus, pinout),
            ...
            0x6702 => pinout = zeropage_modify_c2::<Rra>(&mut self.cpu, bus, pinout),
            ...
            0x7703 => pinout = zeropage_x_modify_c3::<Rra>(&mut self.cpu, bus, pinout),
            ...
            0x6F03 => pinout = absolute_modify_c3::<Rra>(&mut self.cpu, bus, pinout),
            ...
            0x7F04 => pinout = absolute_x_modify_c4::<Rra>(&mut self.cpu, bus, pinout),
            ...
            0x7B04 => pinout = undoc_absolute_y_c4::<Rra>( &mut self.cpu, bus, pinout),
            ...
            0x6305 => pinout = undoc_indirect_x_c5::<Rra>( &mut self.cpu, bus, pinout),
            ...
            0x0702 => pinout = zeropage_modify_c2::<Slo>(&mut self.cpu, bus, pinout),
            ...
            0x1703 => pinout = zeropage_x_modify_c3::<Slo>(&mut self.cpu, bus, pinout),
            ...
            0x0F03 => pinout = absolute_modify_c3::<Slo>(&mut self.cpu, bus, pinout),
            ...
            0x1F04 => pinout = absolute_x_modify_c4::<Slo>(&mut self.cpu, bus, pinout),
            ...
            0x1B04 => pinout = undoc_absolute_y_c4::<Slo>( &mut self.cpu, bus, pinout),
            ...
            0x0305 => pinout = undoc_indirect_x_c5::<Slo>( &mut self.cpu, bus, pinout),
            ...
            0x1305 => pinout = undoc_indirect_y_c5::<Slo>( &mut self.cpu, bus, pinout),
            ...
            0x4702 => pinout = zeropage_modify_c2::<Sre>(&mut self.cpu, bus, pinout),
            ...
            0x5703 => pinout = zeropage_x_modify_c3::<Sre>(&mut self.cpu, bus, pinout),
            ...
            0x4F03 => pinout = absolute_modify_c3::<Sre>(&mut self.cpu, bus, pinout),
            ...
            0x5F04 => pinout = absolute_x_modify_c4::<Sre>(&mut self.cpu, bus, pinout),
            ...
            0x5B04 => pinout = undoc_absolute_y_c4::<Sre>( &mut self.cpu, bus, pinout),
            ...
            0x4305 => pinout = undoc_indirect_x_c5::<Sre>( &mut self.cpu, bus, pinout),
            ...
            0x5305 => pinout = undoc_indirect_y_c5::<Sre>( &mut self.cpu, bus, pinout),
            ...
            0x9E03 => pinout = absolute_y_store_c3::<Sxa>(&mut self.cpu, bus, pinout),
            ...
            0x9C03 => pinout = absolute_x_store_c3::<Sya>(&mut self.cpu, bus, pinout),
            ...
            0x0C03 =>  pinout = absolute_read_c3::<Nop>(&mut self.cpu, bus, pinout),
            // Nop absolute x read
            ...
            0x1C04 =>  pinout = absolute_x_read_c4::<Nop>(&mut self.cpu, bus, pinout),
            // Nop absolute x read
            ...
            0x3C04 =>  pinout = absolute_x_read_c4::<Nop>(&mut self.cpu, bus, pinout),
            // Nop absolute x read
            ...
            0x5C04 =>  pinout = absolute_x_read_c4::<Nop>(&mut self.cpu, bus, pinout),
            // Nop absolute x read
            ...
            0x7C04 =>  pinout = absolute_x_read_c4::<Nop>(&mut self.cpu, bus, pinout),
            // Nop absolute x read
            ...
            0xDC04 =>  pinout = absolute_x_read_c4::<Nop>(&mut self.cpu, bus, pinout),
            // Nop absolute x read
            ...
            0xFC04 =>  pinout = absolute_x_read_c4::<Nop>(&mut self.cpu, bus, pinout),
            // Xaa immediate
            0x8B00 =>  pinout = immediate_read_c0(&mut self.cpu, bus, pinout),
            0x8B01 =>  pinout = immediate_read_c1::<Xaa>(&mut self.cpu, bus, pinout),
            ...
            0x9B03 => pinout = absolute_y_store_c3::<Xas>(&mut self.cpu, bus, pinout),
    ...
    pub fn debug_log(&self) -> String {
        let log = format!("{:#X}  IR:{:#X} TM:{:#X} SYNC:{} {} - A:{:#X} X:{:#X} Y:{:#X} P:{:#X} SP:{:#X} CYC: {}",
